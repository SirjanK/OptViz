...
void initialize_ego_viz_module(ModuleInitializationLevel p_level) {
...
void uninitialize_ego_viz_module(ModuleInitializationLevel p_level) {
    if (p_level != MODULE_INITIALIZATION_LEVEL_SCENE) {
        return;
    }
}
...
extern "C" {
    GDExtensionBool GDE_EXPORT ego_viz_library_init(GDExtensionInterfaceGetProcAddress p_get_proc_address, GDExtensionClassLibraryPtr p_library, GDExtensionInitialization *r_initialization) {
        ...
        init_obj.register_initializer(initialize_ego_viz_module);
        init_obj.register_terminator(uninitialize_ego_viz_module);
